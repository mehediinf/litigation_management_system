//lib/features/legal_team/warrant_of_arrest/presentation/views/warrant_of_arrest_process.dart

import 'package:flutter/material.dart';
import 'package:dropdown_search/dropdown_search.dart';

import 'arrest_process/arrest_action_delete_page.dart';
import 'arrest_process/arrest_action_edit_page.dart';
import 'arrest_process/arrest_action_preview_page.dart';
import 'arrest_process/arrest_action_send_to_checker_page.dart';
import 'arrest_process/arrest_action_verify_page.dart';

class WarrantOfArrestProcessPage extends StatefulWidget {
  const WarrantOfArrestProcessPage({super.key});

  @override
  State<WarrantOfArrestProcessPage> createState() =>
      _WarrantOfArrestProcessPageState();
}

class _WarrantOfArrestProcessPageState extends State<WarrantOfArrestProcessPage>
  ...
  final TextEditingController _serialNoSearchController =
  ...
  void dispose() {
    ...
    _serialNoSearchController.dispose();
  ...
  void _showAdvancedSearchDialog() {
    showDialog(
      ...
      builder: (context) => _AdvancedSearchDialog(
        ...
        initialSerialNo: _serialNoSearchController.text,
        onSearch:
            (
              caseType,
              proposedType,
              ...
              setState(() {
                ...
                _serialNoSearchController.text = serialNo;
              });
  ...
  void _openActionByCode(BuildContext context, String code) {
    ...
    switch (code) {
      case 'D':
        Navigator.of(context).push<void>(
          MaterialPageRoute<void>(
            builder: (_) => ArrestActionDeletePage(caseData: d),
          ),
        );
        return;
      case 'E':
        Navigator.of(context).push<void>(
          MaterialPageRoute<void>(
            builder: (_) => ArrestActionEditPage(caseData: d),
          ),
        );
        return;
      case 'STC':
        Navigator.of(context).push<void>(
          MaterialPageRoute<void>(
            builder: (_) => ArrestActionSendToCheckerPage(caseData: d),
          ),
        );
        return;
      case 'V':
        Navigator.of(context).push<void>(
          MaterialPageRoute<void>(
            builder: (_) => ArrestActionVerifyPage(caseData: d),
          ),
        );
        return;
      ...
      case 'P':
        Navigator.of(context).push<void>(
          MaterialPageRoute<void>(
            builder: (_) => ArrestActionPreviewPage(caseData: d),
          ),
        );
        return;
...
class _AdvancedSearchDialogState extends State<_AdvancedSearchDialog> {
  ...
  void initState() {
    ...
    _serialNoController.text = widget.initialSerialNo;
  }
...
final List<Map<String, dynamic>> _dummyCases = [
  {
    'Actions': ['D', 'E', 'STC', 'V', 'P'],
    'Status': 'Verified',
    'Current Case Status': 'Auction 33(4)',
    'Region': 'Central',
    'District': 'Bagerhat',
    'Branch Name': 'DINAJPUR SME/KRISHI BR(1601)',
    'Case Type': 'ARA-2003',
    'Case Number': 'Artho-50/22',
    'Account Name': 'Akkas Sari Bitan',
