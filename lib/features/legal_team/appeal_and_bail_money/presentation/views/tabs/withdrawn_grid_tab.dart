...
class WithdrawnGridTab extends StatelessWidget {
  ...
  static final List<Map<String, dynamic>> _allData = [
    {
      'Actions': ['D', 'E', 'STC', 'V', 'P'],
      'Account Name': 'Choudhuri Transport',
      'Account': '5501603448162001',
      'Case Number': 'CR-1211/17(SC-689/18)(Appeal-32...)',
      'Withdraw Date': '08/10/2025',
      ...
      child: Column(
        ...
        children: [
          ...
          if (data.isEmpty)
            ...
            ...data.map((row) => _WithdrawnCard(data: row)),
        ],
      ),
    );
  }
...
class _WithdrawnCard extends StatefulWidget {
  const _WithdrawnCard({required this.data});

  final Map<String, dynamic> data;

  @override
  State<_WithdrawnCard> createState() => _WithdrawnCardState();
}

class _WithdrawnCardState extends State<_WithdrawnCard> {
  ...
  bool get _isNegativeStatus {
    ...
    return s.contains('rejected') || s.contains('decline');
  }
  ...
  List<_WithdrawnGridActionEntry> _buildActionEntries(BuildContext context) {
    var i = 0;
    final list = <_WithdrawnGridActionEntry>[];
        ...
        child: Stack(
          ...
          children: [
                ...
                children: [
                  Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        Expanded(
                          child: _WithdrawnStatusHeaderChip(
                            statusText: _v('status'),
                            isNegative: _isNegativeStatus,
                          ),
                        ),
                        const SizedBox(width: 70),
                        _WithdrawnGridActionToggle(
                          open: _showActions,
            ...
            AnimatedPositioned(
              ...
              bottom: 0,
              width: 200,
              child: _WithdrawnActionsSlidePanel(
...
class _WithdrawnStatusHeaderChip extends StatelessWidget {
  const _WithdrawnStatusHeaderChip({
...
class _WithdrawnDataFieldRow extends StatelessWidget {
  const _WithdrawnDataFieldRow({required this.label, required this.value});
...
class _WithdrawnGridActionToggle extends StatelessWidget {
  const _WithdrawnGridActionToggle({
    required this.open,
  ...
  static const Color _kGreen = Color(0xFF43A047);
        ...
        children: [
          AnimatedContainer(
            ...
            decoration: BoxDecoration(
              ...
              color: open ? _kGreen : Colors.white,
              border: Border.all(
                color: open ? const Color(0xFF2E7D32) : const Color(0xFFE0E5F0),
                width: 1.4,
              ),
              boxShadow: [
                BoxShadow(
            ...
            child: Center(
              child: AnimatedSwitcher(
                ...
                child: Icon(
                  ...
                  size: 18,
                  color: open ? Colors.white : _kGreen,
                ),
              ),
            ),
          ),
...
class _WithdrawnActionsSlidePanel extends StatelessWidget {
  const _WithdrawnActionsSlidePanel({
  ...
  final Color primary;
  final List<_WithdrawnGridActionEntry> entries;
        ...
        children: [
          ...
          Expanded(
            child: entries.isEmpty
                ? const _WithdrawnActionsEmptyHint()
                : Scrollbar(
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (_, i) => Padding(
                        ...
                        child: _WithdrawnCompactActionTile(entry: entries[i]),
                      ),
                    ),
                  ),
          ),
...
class _WithdrawnActionsEmptyHint extends StatelessWidget {
  const _WithdrawnActionsEmptyHint();
...
class _WithdrawnGridActionEntry {
  const _WithdrawnGridActionEntry({
    required this.label,
...
class _WithdrawnCompactActionTile extends StatefulWidget {
  const _WithdrawnCompactActionTile({required this.entry});

  final _WithdrawnGridActionEntry entry;

  @override
  State<_WithdrawnCompactActionTile> createState() =>
      _WithdrawnCompactActionTileState();
}

class _WithdrawnCompactActionTileState extends State<_WithdrawnCompactActionTile>
